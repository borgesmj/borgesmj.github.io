...
console.log(currentPath)
  ...
  return (
    <nav className="navbar bg-background-100 w-dvw text-white py-2 px-1 bottom-0 left-0 z-50 fixed md:left-12 md:h-full md:w-[13rem] md:top-0 md:bottom-8 md:border-r-[1px] md:border-r-solid md:border-t-none md:border-r-white border-r-accent-100 md:flex md:flex-col md:justify-center lg:left-16  font-mono xl:left-28 2xl:left-60">
      <ul className=" flex flex-row w-full h-full justify-between md:flex-col md:justify-around px-0 md:h-1/4 lg:h-1/3">
        {navLinks.map((item, index) => (
          <li key={index} className={`flex justify-center items-center md:p-2 w-1/5 ${currentPath === item.href ?  "border-t-primary-100" : "border-t-white"} ${currentPath === item.href ?  "border-t-[4px]" : "border-t-[2px]"} ${currentPath === item.href ?  "border-l-primary-100" : "border-l-transparent"} ${currentPath === item.href ?  "border-l-[4px]" : "border-l-[2px]"} border-t-solid py-2 md:border-t-[0px] md:border-t-white md:border-t-solid md:border-t-[1px] md:w-full md:h-1/5 lg:relative`}>
